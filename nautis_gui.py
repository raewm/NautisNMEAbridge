...
class NautisGuiWindow(QMainWindow):
    ...
    def _build_ap_settings_group(self) -> QGroupBox:
        ...
        self._preset_labels = ["Slow", "Medium", "Fast"]
        self._preset_slider = QSlider(Qt.Horizontal)
        self._preset_slider.setMinimum(0)
        self._preset_slider.setMaximum(2)
        self._preset_slider.setValue(1)   # Medium default
        self._preset_slider.setTickPosition(QSlider.TicksBelow)
        self._preset_slider.setTickInterval(1)
        ...
        """)
        self._preset_slider.valueChanged.connect(self._on_preset_changed)
        lay.addWidget(self._preset_slider, 5, 0, 1, 2)

        # Slider label row: Slow / Medium / Fast
        lbl_row = QHBoxLayout()
        lbl_row.setContentsMargins(0, 0, 0, 0)
        for name in self._preset_labels:
            l = _label(name, size=10)
            l.setAlignment(Qt.AlignCenter)
            lbl_row.addWidget(l)
        lay.addLayout(lbl_row, 6, 0, 1, 2)

        self._preset_name_lbl = _label("MEDIUM", color=ACCENT, bold=True)
    ...
    def _build_footer(self) -> QWidget:
        ...
        self._version_lbl = _label("v2.0  |  © NAUTIS Home NMEA Bridge", color=TEXT_DIM)
    ...
    def _on_preset_changed(self, idx: int):
        name = self._preset_labels[idx]
        self._preset_name_lbl.setText(name.upper())
        if self._engine:
            self._engine.apply_vessel_preset(name)
        else:
            # Update the Advanced PID fields so the user can see the values
            from autopilot import VESSEL_PRESETS
            kp, ki, kd, lim = VESSEL_PRESETS[name]
            self._kp.setText(str(kp))
            self._ki.setText(str(ki))
            self._kd.setText(str(kd))
