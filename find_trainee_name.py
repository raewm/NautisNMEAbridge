"""
find_trainee_name.py -- check if vstep.trainees.Name exists in the registry
"""
import os, sys, time
import grpc
...
def main():
    ...
    try:
        ...
        print("Trainee Name Components in Registry:")
        found = False
        for eid, comps in sorted(entities.items()):
            if "vstep.trainees.Name" in comps:
                found = True
                tn = comps["vstep.trainees.Name"]
                disp = comps["vstep.entities.DisplayName"].name if "vstep.entities.DisplayName" in comps else "Unknown"
                mmsi = comps["vstep.equipment.MMSI"].identifier if "vstep.equipment.MMSI" in comps else "No MMSI"
                print(f"  Entity {eid} ('{disp}', MMSI: {mmsi}): computer_name = {tn.computer_name}, user_name = {tn.user_name}")
        if not found:
            print("  None found!")
