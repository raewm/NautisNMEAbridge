...
**Current version: `2.1.0`** — see [Changelog](#changelog) for history and exe vs. script comparison.
...
The version string is defined in `nautis_nmea_bridge.py` as `__version__` and displayed
in the GUI footer. **Bump `__version__` every time a new `.exe` is built** so the script
and executable versions are always easy to compare.
...
| **1.0.0** | 2026-06-01 | ✅ | Initial release — gRPC polling bridge, NMEA + AIS output, deadlock-free architecture. |

### How to release a new exe

1. Make and test your code changes in `nautis_nmea_bridge.py`.
2. Bump `__version__` in `nautis_nmea_bridge.py` and add a row to the table above.
3. Rebuild the executable:
   ```
   pyinstaller --clean nautis_nmea_bridge.spec
   ```
4. Verify the GUI footer shows the new version number.
